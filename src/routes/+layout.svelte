...
<svelte:head>
  <script
    async
    src="https://www.googletagmanager.com/gtag/js?id=G-DDJB7B7GPW"
  ></script>
  <script>
  ...
  <meta
    ...
    content="A multimedia collage zine type of thing, a memoir with songs."
  />
  <meta
    name="twitter:image"
    content="https://roosterking.drewschorno.com/img/twitter-image.jpg"
  />
  ...
  <meta
    property="og:image"
    content="https://roosterking.drewschorno.com/img/og-image.jpg"
  />
...
>
  <a href="/">ROOSTER KING</a>
  <ul>
    {#each pages as page}
      <li>
        <a class:active={data.pathname == page.route} href={page.route}
          >{page.name}</a
        >
      </li>
    {/each}
...
{#key data.pathname}
  ...
  <video
    ...
    class="canary autoplay"
    loop
    bind:paused={canaryPaused}
    style="display: none;"
  ></video>
{/key}
  ...
  >
    <div class="clickModal__inner">
      {#if clickModalEscalations === 0}
        <p>
          Halt! By virtue of the divine edict set forth by the Apple
          Corporation, you must surrender one click.
        </p>
        <button>Of course, my leige...</button>
        <button>Away, scoundrel!</button>
      {:else}
        <p>Another!</p>
