...
<div
  class="cover autolayout"
  style="--left:.5fr; width:calc(43% - 5vw); overflow: hidden;"
>
...
<style type="text/css">
  ...
  .eye_anim {
    ...
    animation: calc(var(--frames) / var(--fps) * 1000ms)
      steps(calc(var(--frames) - 1)) 0s infinite filmstrip;
    position: fixed;
    top: 39%;
    left: 43%;
    height: 90vh;
  ...
  h1.title {
    ...
    color: #b7955f;
    animation:
      10s linear infinite alternate hueshift,
      5s title__blurin both;
    background: #000;
  }

  @keyframes hueshift {
    0% {
      color: oklch(0.8 0.17 0);
    }
    25% {
      color: oklch(0.8 0.17 120);
    }
    50% {
      color: oklch(0.8 0.17 240);
    }
    100% {
      color: oklch(0.8 0.17 360);
    }
  }
